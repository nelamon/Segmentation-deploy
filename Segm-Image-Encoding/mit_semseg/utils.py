    ...
    for label in unique(labelmap):
        ...
        if label==90:
            labelmap_rgb += (labelmap == label)[:, :, np.newaxis] * \
                np.tile(np.array([0,0,0],dtype='uint8'),
...
def accuracy(preds, label):
    ...
    acc_pred=np.multiply(label,((preds == label)*1))
    
    valid_sum = valid.sum()
    acc = float(acc_sum) / (valid_sum + 1e-10)
    
    uniques_acc, counts_acc = np.unique(acc_pred, return_counts=True)
    uniques_pred, counts_pred = np.unique(preds, return_counts=True)
    
    acc_list=np.zeros((151,1))
    pred_list=np.zeros((151,1))
    
    for index,item in enumerate(uniques_acc):
        acc_list[item]=counts_acc[index]
    
    for index,item in enumerate(uniques_pred):
        pred_list[item]=counts_pred[index]
    
    return acc, valid_sum, acc_list,pred_list
